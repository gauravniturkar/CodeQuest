...
echo ""
echo "3. Deploy:"
echo "   The site is now a Vite build — serving the repo root will not work."
echo "   → Vercel (recommended, keeps the api/ functions):"
echo "       vercel        # auto-detects Vite, publishes dist/"
echo "   → GitHub Pages: build first, then publish dist/"
echo "       npm run build && npx gh-pages -d dist"
echo "       (the AI endpoints need a serverless host; Pages falls back to the"
echo "        built-in question bank)"
echo ""
